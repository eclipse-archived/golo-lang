...
class ParseTreeToGoloIrVisitor implements GoloParserVisitor {
  ...
  public Object visit(ASTFunctionDeclaration node, Object data) {
    ...
    GoloFunction function = new GoloFunction(
        node.getName(),
        node.isLocal() ? GoloFunction.Visibility.LOCAL : GoloFunction.Visibility.PUBLIC,
        node.isAugmentation() ? GoloFunction.Scope.AUGMENT : GoloFunction.Scope.MODULE);
    if (node.isDecorator()) {
  ...
  public Object visit(ASTFunction node, Object data) {
    ...
    if (isSynthetic) {
      function = new GoloFunction(
          ...
          GoloFunction.Visibility.LOCAL,
          GoloFunction.Scope.CLOSURE);
    ...
    if (GoloFunction.Scope.AUGMENT.equals(function.getScope())) {
  ...
  private Block createCollectionComprehension(Context context, String type, List<ExpressionStatement> expressions) {
    ...
    LocalReference collectionRef = new LocalReference(LocalReference.Kind.VARIABLE, collectionName, true);
  ...
  public Object visit(ASTAssignment node, Object data) {
    ...
    if (reference == null) {
      getOrCreateExceptionBuilder(context).report(GoloCompilationException.Problem.Type.UNDECLARED_REFERENCE, node,
  ...
  public Object visit(ASTDestructuringAssignment node, Object data) {
    ...
    LocalReference destructReference = new LocalReference(LocalReference.Kind.CONSTANT, varName, true);
    ...
    for (String name : node.getNames()) {
      LocalReference val;
      if (declaring) {
        val = new LocalReference(node.getType() == ASTLetOrVar.Type.LET ? LocalReference.Kind.CONSTANT : LocalReference.Kind.VARIABLE, name, true);
        parentTable.add(val);
      } else {
    ...
    for (i = 0; i < numChildren; i++) {
      ...
      if (statement instanceof NamedArgument) {
        if (!functionInvocation.getArguments().isEmpty() && !functionInvocation.usesNamedArguments()) {
          getOrCreateExceptionBuilder(context).report(GoloCompilationException.Problem.Type.INCOMPLETE_NAMED_ARGUMENTS_USAGE, node,
  ...
  public Object visit(ASTAnonymousFunctionInvocation node, Object data) {
    ...
    if (node.isOnExpression()) {
      ...
      context.objectStack.push(new BinaryOperation(OperatorType.ANON_CALL, receiver, invocation));
    } else {
  ...
  public Object visit(ASTForEachLoop node, Object data) {
    ...
    if (node.getElementIdentifier() != null) {
      ...
      LocalReference destructReference = new LocalReference(LocalReference.Kind.VARIABLE, tmpName, true);
      localTable.add(destructReference);
      ...
      int last = node.getNames().size() - 1;
      for (String name : node.getNames()) {
        LocalReference val = new LocalReference(LocalReference.Kind.VARIABLE, name, true);
        MethodInvocation get = new MethodInvocation(!node.isVarargs() || idx != last ? "get" : "subTuple");
        get.addArgument(new ConstantStatement(idx));
        localTable.add(val);
        AssignmentStatement valInit = new AssignmentStatement(val,
  ...
  public Object visit(ASTTryCatchFinally node, Object data) {
    ...
    if (hasCatchBlock) {
      ...
      catchBlock.getReferenceTable().add(new LocalReference(LocalReference.Kind.CONSTANT, exceptionId, true));
    } else {
