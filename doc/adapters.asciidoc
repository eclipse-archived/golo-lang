...
function main = |args| {
  let sparkRouteAdapter = Adapter()   # <1>
    : extends("spark.Route")   # <2>
    : implements("handle", |this, request, response| {   # <3>  # <4>
        return "Golo, world!"
      })
  let route = sparkRouteAdapter: newInstance("/hello")  # <5>
  get(route)  # <6>
...
<4> The implementation is given by a closure whose signature matches the parent class definition,
    and where the first argument is the receiver object that is going to be the adapter instance.
<5> The `newInstance()` method calls the right constructor based on the parent class constructors
    and provided argument types.
<6> The `spark.Spark.get()` static is method is happy as we feed it a subclass of `spark.Route`.
