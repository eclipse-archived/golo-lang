...
The general syntax is an collection literal containing an expression followed
...
[source,golo]
----
let l = list[ 3 * x + 1 for (var x=0, i < 10, i = i + 2) ]
----
...
However, the result collection can only be one of the predefined collection
literals.
...
Maps can also be created, provided the given expression is either a 2-tuple or
...
HOF on a collection. Indeed, a comprehension such as:
[source,golo]
----
...
Thus, should you use collection comprehension or HOF? Despite some
