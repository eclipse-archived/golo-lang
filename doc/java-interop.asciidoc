...
=== Spread method invocation ===

Golo is able to spread a method call on all items of an iterable object.

[source,text]
------------------------------------------------------
["Golo", "is", "cool"]*: toUpperCase()
# equivalent to
["Golo", "is", "cool"]: map(|it| -> it: toUpperCase())
------------------------------------------------------

The spread invocation returns a *`gololang.Tuple`* aggregating the results of each unitary invocation call.
