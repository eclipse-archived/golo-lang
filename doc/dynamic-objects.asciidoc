...
- `fallback(value)` defines a fallback behavior for property invocation.
...
The `fallback(value)` method let the user define a method invoked whenever the initial method dispatch failed.
Here is an example of how define a fallback.
...
fallback(|this, method, args...| -> "Dispatch failed for method: " + method + ", with args:" + args: asList(): join(" "))

println(dynob: casperGetter())
#Setter-like invocation creates the property, thus the fallback can't be invoked.
println(dynob: casperSetter("foo"))
