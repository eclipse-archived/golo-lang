...
NOTE: Higher order functions are functions that process functions, i.e. that take 
...
of its arity (see examples below).
...
functionality is provided by the `gololang.Decorators.withContext` standard
decorator. This decorator take a context, such as the one returned by
